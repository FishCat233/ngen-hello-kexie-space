...
- **移动端菜单展开动画 + 全屏遮罩（2026-09-22 用户指令）**：展开动画为**「底边推进」**——面板底边自导航栏下缘向下生长至最终位置：`clip-path: inset(0 0 100% 0) → inset(0)` 揭示（面板为 absolute 悬浮面板、高度无法像 grid `0fr→1fr` 那样插值，clip 揭示是等价实现且面板每帧尺寸真实），0.32s `cubic-bezier(0.34, 1.2, 0.5, 1)` 过冲曲线，内容随底边推进同步 `-6px` 下移淡入；出场 0.2s ease 反向收起。**关键坑：终态必须显式 `clip-path: inset(0)`**——默认值 `none` 与形状之间不做连续插值（离散跳变、动画中段瞬切，2026-09-22 实测踩坑），两个 `inset()` 之间才平滑推进。**全屏遮罩 `.navbar-overlay`**（`rgba(0,0,0,0.5)` 压暗页面 + 0.25s 淡入淡出）**作为 `<nav>` 的兄弟节点渲染**（关键坑：nav 带 `transform: translateX(-50%)`，其内部 `position: fixed` 后代会以 nav 为包含块而非视口——遮罩必须逃出 nav）；`z-index: 99`**——盖住全部页面内容、低于 nav(100)，菜单/X 按钮/Logo 保持可点；点击遮罩任意空白处关闭菜单（X 按钮仍有效）。全站层级参考：RouteProgress 200、灯箱 2000 不受影响
