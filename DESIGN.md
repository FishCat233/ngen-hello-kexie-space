...
| `--color-card` | `#1d1d1d` | 二级卡片：比背景浅一档的纯灰，表达层级（2026-09-20 用户指令替代蓝色描边；2026-09-21 去蓝色调改纯灰） |
...
- 移动端菜单（2026-09-22 用户指令重调）：**悬浮磨砂玻璃面板**，`absolute` 定位于导航栏下方 12px；**宽度恒等于药丸态导航栏** `min(calc(100vw - 32px), 1080px)`（2026-09-22 二次修订：通栏状态打开时不再全宽、两侧各留 16px，药丸状态打开时严格同宽）——用 `100vw` 锚定视口而非 `100%`（100% 随 navbar 当前宽度漂移），居中用 `left/right: 0 + width + margin: 0 auto`（不用 `left: 50% + translateX(-50%)`，避免与展开动画的 transform 关键帧冲突）；**无分割线**、条目间距收紧（面板 padding 12px、条目 gap 2px、链接行 padding 10px 18px）；**圆角 32px** = 导航栏药丸态的「有效圆角」（999px 被半高钳制后的视觉值），与加入我们 CTA 视觉协调——不用字面 `--radius-pill`（多行面板变体育场形，首尾条目被圆角曲线裁切）；玻璃配方 `rgba(10,14,20,0.6)` + `blur(20px) saturate(150%)`；条目行与 CTA 同为药丸圆角。窄屏右侧的**汉堡菜单按钮无描边**（2026-09-21 用户指令：40px 透明底白图标、去主题蓝描边，hover 主题蓝填充不变）
- **移动端菜单展开动画 + 全屏遮罩（2026-09-22 用户指令）**：展开动画为**纯「底边推进」生长**（三修定稿）——面板底边自导航栏下缘向下生长至最终位置，**仅 `clip-path: inset(0 0 100% 0) → inset(0)` 一项在动，无位移、无透明度变化**；入场 0.38s `cubic-bezier(0.22, 1, 0.36, 1)`（easeOutQuint：起步快、收尾绵长丝滑、无过冲弹跳），出场 0.24s `cubic-bezier(0.4, 0, 1, 1)` easeIn 反向收起（面板为 absolute 悬浮面板、高度无法像 grid `0fr→1fr` 那样插值，clip 揭示是等价实现且面板每帧尺寸真实）。**关键坑：终态必须显式 `clip-path: inset(0)`**——默认值 `none` 与形状之间不做连续插值（离散跳变、动画中段瞬切，2026-09-22 实测踩坑），两个 `inset()` 之间才平滑推进。**全屏遮罩 `.navbar-overlay`**（`rgba(0,0,0,0.5)` 压暗页面 + 0.25s 淡入淡出）**作为 `<nav>` 的兄弟节点渲染**（关键坑：nav 带 `transform: translateX(-50%)`，其内部 `position: fixed` 后代会以 nav 为包含块而非视口——遮罩必须逃出 nav）；`z-index: 99`**——盖住全部页面内容、低于 nav(100)，菜单/X 按钮/Logo 保持可点；点击遮罩任意空白处关闭菜单（X 按钮仍有效）。全站层级参考：RouteProgress 200、灯箱 2000 不受影响
...
草案（2026-09-20，随首屏改版引入）：**`>` 路径符号 + `[ ]` 方括号**——首屏第一行「在 > [ 科协 ] 部门」中的 `>`（内联 SVG 尖角箭头、primary 色；SVG 不吃 text-shadow，2026-09-21 用户指令补同参数 `drop-shadow(0 0 0.18em rgba(0,0,0,0.55))` 与标题文字同款阴影）与独立于「科协」两侧的方括号（primary 色），接替旧版 `# ` Markdown 语法前缀的识别性角色。待确认是否推广到子页面标题。
...
- **≤1024px 移动端堆叠布局（2026-09-22 用户指令）**：字号脱离桌面 vw 公式（窄屏跌 18px 下限过小），独立 `clamp(30px, 10vw, 56px)`；`hero-content` 内边距收窄至 20px。结构纵向堆叠——第一行拆两行（`在 > [科协]` 一行、药丸卡堆 `flex-basis: 100%` 换行后 `justify-content: center` 整行居中、去 0.18em 左距）；第二行拆两行（「一起」`flex-basis: 100%` 独占一行，三词组按剩余宽度换行——窄屏「两词 + 一词」）。卡堆 em 偏移随新字号等比缩放，观感与桌面一致；词组均为 4 字、行宽恒定，FLIP 在移动端自动退化为 no-op
- **≤1024px 移动端按钮 2×2 网格（2026-09-22 用户指令）**：`hero-buttons` 改 `display: grid` + `grid-template-columns: repeat(2, minmax(0, 1fr))`、gap 10px、容器 `width: min(100%, 360px)`（限宽防宽屏下按钮被拉成超宽药丸）；按钮去固定 130px 宽改 `width: auto` 拉伸填满单元格——flex 换行在中宽平板会挤成一行四个、手机才 2+2，网格统一为恒定 2×2。桌面 >1024px 仍为 flex 一行四个（140px 固定宽）不变
- **≤1024px 垂直定位修复（2026-09-22）**：取消桌面 `-6vh` 上移（`hero-content` `transform: none`）——堆叠标题整体更高，上移会把首行「在 > [科协]」顶进固定导航栏；`.hero-section` 改 `padding-top: 84px`（导航栏 64px + 20px 呼吸）+ `align-items: safe center`，内容在净空以下居中；矮视口（如浏览器设备模拟窄高窗）放不下时 `safe center` 退化为顶部对齐、起点仍在导航栏之下，首行任何视口下不被遮挡。桌面 >1024px 两行标题的 `-6vh` 设计不变
...
- 竖版部门卡（`min(460px, 88vw)` × min-height 480px）：上部 200px 灰色图片预留位（`#d4d4d4`，待放图），下部内容区——部门名 h2 白色、两段正文 body-lg `#b0b0b0`、可选链接 pill（安全部「安全部主页」按钮：2px 白色描边药丸，2026-09-21 用户指令由 1px line 描边加粗换白；hover 蓝底深字不变）
...
**成员卡布局规则（2026-09-22 修复 + 二修）**：`.member-card` 为 wrap 布局卡片，两行 flex 线（信息行 + 链接行 `width: 100%` 独占），网格同行高卡会被 grid 拉伸至等高——卡片用 `align-content: space-between`（2026-09-22 二修：由 `flex-start` 改）：**链接行推到卡片底部**，短介绍卡的按钮与同行大卡底部平齐（页脚式对齐），同时行高保持内容尺寸（默认 `stretch` 会把多余高度分给各行、拉高链接行——禁止）。`.member-links` 加 `align-items: flex-start`，**GitHub 等链接按钮高度跟随内容**（padding 6px 12px），不随卡片/行被拉高填满父级

**二级卡片层级规则（2026-09-20 用户指令，二次修订；2026-09-21 三修：卡片灰去蓝色调）**：全站二级卡片统一 `--color-card`(#1d1d1d) **纯灰**底表达层级（原 #181d26 带蓝色调，用户不要；骨架屏流光同步改纯灰 #1f1f1f→#2c2c2c），**不用蓝色描边**；**hover 一律「变深」**（`#141414` 纯灰），文字与内部元素层级不变，禁止浅蓝色块滑切/填充 hover。已迁移：ProjectCard、CompetitionProjectCard、member/comment/award 卡、gallery 卡（图片缩放与遮罩保留）、cms-notice/empty 提示框、DirectionView markdown 容器、iframe 弹窗（surface + 1px line 细描边）。按钮/标签/头像/图标框的蓝色描边保留（非卡片）；**小号药丸按钮保留亮蓝滑切填充**（成员链接、github-link、direction-link 等）。**导航栏 CTA 滑切块为深蓝 `#2563eb`**（比底色深，2026-09-20 用户指令）；学习方向行为列表行（非卡片），hover 浮现 `--color-card` 底
