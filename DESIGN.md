...
- **首屏 CTA 按钮**（`HeroSection.vue`）同为磨砂玻璃，透出底层 HeroBand 色带：`rgba(10,14,20,0.45)` + `blur(16px) saturate(150%)`，无阴影；描边仍为 2px `--color-primary`，hover 滑切填充不变
...
- 部门专属色（卡片纯色底 + 词组文字，`deptColors` 映射）：多媒体 `#3b82f6` / 软件 `#a78bfa` / 硬件 `#fb923c` / 组织 `#34d399` / 安全 `#f87171`
