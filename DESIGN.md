...
**成员卡布局规则（2026-09-22 修复）**：`.member-card` 为 wrap 布局卡片，网格同行高卡会被 grid 拉伸至等高——卡片必须 `align-content: flex-start`（默认 `stretch` 会把多余高度分给各行，包括链接行）；`.member-links` 加 `align-items: flex-start`，**GitHub 等链接按钮高度跟随内容**（padding 6px 12px），不随卡片/行被拉高填满父级
