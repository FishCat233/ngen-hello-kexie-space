...
> **重构期状态说明**：本文档的**功能与数据规格**（导航结构、数据文件、数据结构、交互行为）继续有效；
> **视觉规格**（设计语言、色调、容器样式）在 UI 重构期间以 [DESIGN.md](../DESIGN.md) 为准。
> 注意：本文「主页面五面滚动结构」为旧版描述，当前代码实际为三面结构（主视觉 / 学习方向 / 加入我们），部门介绍与近年获奖已迁移至独立子页面。待页面级规格定稿后统一修订本文档。
...
- 背景: 白色 `#FFFFFF`，叠加淡灰色（`#D0D0D0`）虚线方格（120px 间距，1px 线宽，短线段 dashed）
